...
def resolve_device_config():
    ...
    return Config(batch_size=8, device=device, precision="fp32", timeout_s=10, retries=1)
...
def test_paired_vs_shuffled_scores():
    ...
    mismatch_records = [Record(url=u, caption=c) for u, c in zip(df["url"], shuffled_captions)]
    mismatch_scores = [r.score for r in score_pairs(mismatch_records, model, processor, cfg)]
...
def test_top_1_retrieval_on_mini_set():
    ...
    """
    mini = pd.DataFrame([
        {"url": "https://cdn.leonardo.ai/users/70eb2776-7757-4992-9980-55e780931fd8/generations/46cefe40-b107-4340-a73c-88a9af2455cb/Absolute_Reality_v16_A_photorealistic_of_Santa_Claus_walking_a_0.jpg", "caption": "A photorealistic of Santa Claus walking alone on the surface of the moon at night."},
        {"url": "https://cdn.leonardo.ai/users/441e30e9-fe13-4f22-acaf-838135876ab7/generations/db00712b-deaf-463b-ba3a-759339dcf612/3D_Animation_Style_Cute_koala_bear_eating_bamboo_in_a_jungle_0.jpg", "caption": "Cute koala bear eating bamboo in a jungle"},
        {"url": "https://cdn.leonardo.ai/users/b81d5101-fb09-4410-9f47-b8b9ed1335df/generations/176b3af7-3cb7-497d-8d8f-3bd14a7e4c01/Default_A_pile_of_bricks_on_the_floor_2.jpg", "caption": "A pile of bricks on the floor"},
    ])
    ...
    assert acc == 1.0, "Each caption should retrieve its matching image in this mini-set."
...
def test_caption_quality_ablation():
    ...
    assert scores[0] >= scores[1] > scores[2] - 1e-6, "On-topic captions should score higher than wrong captions."
